...
func unmanagedEgressGraph() *graph.Store {
	...
	g.AddEvent(egress(ebpfcapture.Identity("python3", 0), "api.openai.com:443"))
	// eBPF-only identity reaching an unremarkable host -> medium
	g.AddEvent(egress(ebpfcapture.Identity("curl", 0), "203.0.113.5:443"))
	// eBPF-only identity with no egress events at all -> nothing to flag
	g.AddIdentity(model.Identity{ID: ebpfcapture.Identity("idle", 0), Source: "ebpf"})
...
func TestUnmanagedEgress(t *testing.T) {
	...
	if a, ok := got[ebpfcapture.Identity("python3", 0)]; !ok {
	...
	if a, ok := got[ebpfcapture.Identity("curl", 0)]; !ok {
	...
	if _, ok := got[ebpfcapture.Identity("idle", 0)]; ok {
